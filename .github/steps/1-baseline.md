...
4. Run `npm run lab -- doctor`, then `npm run lab -- test both`.
5. Read the two project files, database configuration, photo services, and tests. Existing tests are a baseline, not proof of successful cloud migration.
6. Read [Copilot guidance](https://github.com/{{repository}}/blob/{{branch}}/.github/copilot-instructions.md) and the provided coach/skill. Create `.github/agents/team-modernizer.agent.md` with `description`, explicit `tools`, and human-approval boundaries. Keep assessment read-only; require approval before edits, migrations, or provisioning.
...
```text
Inspect apps/dotnet and apps/java without modifying them. Explain the actual
frameworks, authentication, storage, database assumptions, and test coverage.
Identify what is known from source versus what needs a runtime check.
Do not provision Azure resources or rewrite application behavior.
```
